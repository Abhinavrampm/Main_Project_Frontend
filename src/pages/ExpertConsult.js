...
const ExpertConsult = () => {
  ...
  useEffect(() => {
    const fetchAdminUsers = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/auth/getAllAdminUsers");
  ...
  const handleUserSelect = async (user) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/auth/messages/${user._id}`);
      setMessages(response.data);

      await axios.post(`http://localhost:5000/api/auth/markMessagesAsSeen/${user._id}`);
    } catch (error) {
    ...
    if (imageUrl != null) {
      try {
        const response = await axios.post(
          `http://localhost:5000/api/auth/sendMessage/${selectedUser._id}`,
