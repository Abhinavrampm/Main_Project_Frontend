...
const ViewJob = () => {
    ...
    const fetchTransactions = async () => {
        try {
            const response = await axios.get("http://localhost:5000/api/auth/getAllApplicantList");
    ...
    const handleSave = async () => {
        try {
            const response = await axios.post("http://localhost:5000/api/auth/postJob", formData);
            Swal.fire({
                title: "Success",
    ...
    const handleApply = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = axios.post(`http://localhost:5000/api/auth/approveAppliant/${id}`);
                    Swal.fire({
                        title: "Success",
    ...
    const handleReject = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = axios.post(`http://localhost:5000/api/auth/rejectAppliant/${id}`);
                    Swal.fire({
                        title: "Success",
    ...
    const handleRelive = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = axios.post(`http://localhost:5000/api/auth/reliveAppliant/${id}`);
                    Swal.fire({
                        title: "Success",
    ...
    const handleHistory = async (id) => {
        try {
            const response = await axios.get(`http://localhost:5000/api/auth/getWorkHistory/${id}`);
    ...
    return (
        <div style={{ padding: "20px", position: "relative", paddingTop: "80px" }}>
            <div
                style={{
                    background: "white",
                    padding: "20px",
                                ...
                                <td style={tableCellStyle}>
                                    {jobs.status === 1 ? (
                                        <button
                                            ...
                                            onClick={() => handleRelive(jobs.jobID)}
                                        >
                                            Relive Applicant
                                        </button>
                                    ) : (
                                        <>
                                            <button
                                                ...
                                                onClick={() => handleApply(jobs.jobID)} // Pass the job ID
                                            >
                                                Approve
                                            </button>
                                            ...
                                            <button
                                                ...
                                                onClick={() => handleReject(jobs.jobID)} // Use handleReject instead of handleApply
                                            >
                                                Reject
                                            </button>
                                            <button
