...
const ViewJob = () => {
    ...
    const fetchTransactions = async () => {
        try {
            const response = await axios.get("http://localhost:5000/api/auth/getAllJobPostings");
    ...
    const handleApply = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = axios.post(`http://localhost:5000/api/auth/applyForJob/${id}`);
                    Swal.fire({
                        title: "Success",
                                ...
                                <td style={tableCellStyle}>
                                    {jobs.countStatus === 1  ? (
                                        ...
                                        <button
                                            ...
                                            onClick={() => handleApply(jobs._id)} // Pass the job ID
                                        >
                                            Apply
                                        </button>
                                    )}
